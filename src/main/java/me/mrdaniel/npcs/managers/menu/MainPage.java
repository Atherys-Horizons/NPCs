...
public class MainPage extends Page {
    ...
    public void updatePage(final Living npc, final NpcFile file) {
        ...
        if (npc.supports(Keys.SKIN_UNIQUE_ID)) {
            lines[++c] = Text.builder().append(Text.of(TextColors.GOLD, "Skin: ", TextColors.AQUA, file.getSkinName().orElse("None"))).onHover(TextActions.showText(Text.of(TextColors.YELLOW, "Change"))).onClick(TextActions.suggestCommand("/npc skin <name>")).build();
        }
        ++c;
        ...
        if (npc.supports(Keys.CREEPER_CHARGED)) {
            lines[++c] = TextUtils.getToggleText("Charged", "/npc charged", file.getCharged());
        }
        if (npc.supports(Keys.ANGRY)) {
            lines[++c] = TextUtils.getToggleText("Angry", "/npc angry", file.getAngry());
        }
        if (npc.supports(Keys.IS_SITTING)) {
            lines[++c] = TextUtils.getToggleText("Sit", "/npc sit", file.getSitting());
        }
        if (npc.supports(Keys.GLOWING)) {
            lines[++c] = TextUtils.getToggleText("Glow", "/npc glow", file.getGlow());
        }
        if (npc.supports(Keys.GLOWING)) {
            lines[++c] = TextUtils.getOptionsText("GlowColor", "/npc glowcolor <color>", file.getGlowColor().map(v -> TextUtils.capitalize(v.getName())).orElse("White"));
        }
        if (npc.supports(Keys.SLIME_SIZE)) {
            lines[++c] = TextUtils.getOptionsText("Size", "/npc size <size>", String.valueOf(file.getSize()));
        }
        if (npc.supports(Keys.CAREER)) {
            lines[++c] = TextUtils.getOptionsText("Career", "/npc career <career>", file.getCareer().map(v -> v.getName()).orElse("None"));
        }
        if (npc.supports(Keys.OCELOT_TYPE)) {
            lines[++c] = TextUtils.getOptionsText("Cat", "/npc cat <cattype>", file.getCat().map(v -> TextUtils.capitalize(v.getId().toLowerCase().replace("ocelot", ""))).orElse("None"));
        }
        if (npc.supports(Keys.HORSE_STYLE)) {
            lines[++c] = TextUtils.getOptionsText("Style", "/npc style <style>", file.getHorseStyle().map(v -> TextUtils.capitalize(v.getName().toLowerCase())).orElse("None"));
        }
        if (npc.supports(Keys.HORSE_COLOR)) {
            lines[++c] = TextUtils.getOptionsText("Color", "/npc color <color>", file.getHorseColor().map(v -> TextUtils.capitalize(v.getName().toLowerCase())).orElse("None"));
        }
        if (npc.supports(Keys.LLAMA_VARIANT)) {
            lines[++c] = TextUtils.getOptionsText("Variant", "/npc variant <variant>", file.getVariant().map(v -> TextUtils.capitalize(v.getName().toLowerCase())).orElse("None"));
        }
    }
}