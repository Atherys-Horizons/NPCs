...
public class NPCMenu {
    ...
    public NPCMenu(@Nonnull final Living npc, @Nonnull final NPCFile file) {
        ...
        if (npc instanceof ArmorEquipable) {
            this.pages.put(PageTypes.ARMOR, new ArmorPage(npc, file));
        }
        ...
        return this.getButton(Text.builder().append(
                Text.builder().append(Text.of(TextColors.AQUA, "⬅ "))
                        ...
                        .onClick(TextActions.executeCallback(src -> {
                            ((ActionsPage) this.pages.get(PageTypes.ACTIONS)).addCurrent(-1);
                            this.send((Player) src, PageTypes.ACTIONS);
                        })).build(),
    ...
    public void updateAndSend(@Nonnull final Player p, @Nonnull final PageType page) {
        Optional.ofNullable(this.pages.get(page)).ifPresent(pg -> {
            pg.update(this.npc, this.file);
            this.send(p, pg);
        });
    }
