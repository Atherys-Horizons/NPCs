...
public class ConditionItem extends Condition {
    ...
    public ConditionItem(@Nonnull final ConfigurationNode node) {
        this(node.getNode("ItemType").getValue(obj -> Sponge.getRegistry().getType(ItemType.class, (String) obj)).get(), node.getNode("Amount").getInt(1), node.getNode("Name").getString(null));
    }
        ...
        for (Inventory s : p.getInventory().slots()) {
            ...
            if (item.isPresent()) {
                ...
                if (stack.getItem() == this.type) {
                    if (this.name != null && !this.name.equalsIgnoreCase(stack.get(Keys.DISPLAY_NAME).map(txt -> TextUtils.toString(txt)).orElse(""))) {
                        continue;
                    }
                    i += stack.getQuantity();
                    if (i >= this.amount) {
