...
public class ActionCondition extends Action {
    ...
    public ActionCondition(@Nonnull final ConfigurationNode node) throws ConditionException {
        this(Condition.of(node.getNode("Condition")), node.getNode("GotoFailed").getInt(0), node.getNode("GotoMet").getInt(0), node.getNode("Take").getBoolean(true));
    }
    ...
    public void setGotoFailed(final int goto_failed) {
        this.goto_failed = goto_failed;
    }

    public void setGotoMet(final int goto_met) {
        this.goto_met = goto_met;
    }
    ...
    public void execute(final Npcs npcs, final ActionResult result, final Player p, final NpcFile file) {
        if (this.condition.isMet(p)) {
            ...
            result.setNext(this.goto_failed);
        }
    }
